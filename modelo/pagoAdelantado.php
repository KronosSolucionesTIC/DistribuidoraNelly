...
class PagoAdelantado
{
    ...
    //Trae los pagoAdelantadoes
    public function getPagoAdelantado()
    {
    ...
    //Valida equipos
    public function getValidaEquipos($data)
    {
        $query = "SELECT COUNT(*) as cantidad FROM `listado_equipos` WHERE cod_contrato = '" . $data['cod_contrato'] . "'";
        $result = mysqli_query($this->link, $query);
        $data   = array();
        while ($data[] = mysqli_fetch_assoc($result));
        array_pop($data);
    ...
    //Valida saldo
    public function getValidaSaldo($data)
    {
        $query = "SELECT saldo_pago FROM `pagos` where saldo_pago > 0 AND fecha_fin_pago <= CURDATE() AND cod_contrato = '" . $data['cod_contrato'] . "'";
        $result = mysqli_query($this->link, $query);
        $data   = array();
        while ($data[] = mysqli_fetch_assoc($result));
        array_pop($data);
    ...
    //Meses
    public function getValidaMeses($data)
    {
    ...
    //Select forma pagoAdelantado
    public function getFormaPagoAdelantado()
    {
    ...
    //Consulta cuotas
    public function getConsultaCuotas($data, $dataSaldo)
    {
        $valor = $data['valor'] - $dataSaldo[0]['saldo_deuda'];
        $query = "SELECT SUM(total_equipo) AS total, '" . $valor . "' / SUM(total_equipo) AS cuotas, IF( '" . $valor . "' <=SUM(total_equipo), 0, MOD(  '" . $valor . "', SUM(total_equipo))) AS residuo FROM `listado_equipos` where cod_contrato = '" . $data['cod_contrato'] . "'";
        $result = mysqli_query($this->link, $query);
        $data   = array();
        while ($data[] = mysqli_fetch_assoc($result));
        array_pop($data);
    ...
    //Elimina pagos
    public function eliminaPagos($data)
    {
        $query  = "DELETE FROM `pagos` WHERE estado_pago =1 AND valor_recibido = 0 AND fecha_fin_pago > CURDATE() AND facturado= 0 AND cod_contrato = '" . $data['cod_contrato'] . "'";
        $result = mysqli_query($this->link, $query);
    ...
    //Crea un nuevo pagoAdelantado
    public function insertaPagoAdelantado($data, $dataCuotas)
    {
        $bandera = '';
        $cuotas = ceil($dataCuotas[0]['cuotas']);
        for($i=0; $i<$cuotas; $i++){
            ...
            if($cantidadPagos[0]['cantidad'] == 0){
                ...
                $pagosContrato = $this->getPagoAdelantadosContrato($data);  
            }
    ...
    //Calcula nuevos pagos
    public function getPagoAdelantadosContrato($data)
    {
    ...
    public function getPagos($data)
    {
        $query = "SELECT valor_recibido, valor_tot_pago, cod_pago, equipos.cod_equipo, fecha_ini_pago, fecha_fin_pago, consecutivo, consecutivo_equipo, nom_clase, nom_equipo, desc_tipo_equipos,MONTH(fecha_ini_pago) as num_mes_ini,YEAR(fecha_ini_pago) as anio_ini,DAY(fecha_ini_pago) as dia_ini,MONTH(fecha_fin_pago) as num_mes_fin,YEAR(fecha_fin_pago) as anio_fin,DAY(fecha_fin_pago) as dia_fin, saldo_pago, valor_descuento FROM pagos
                ...
                INNER JOIN clase_equipos ON clase_equipos.cod_clase = equipos.clase_equipo
                INNER JOIN tipo_equipos ON tipo_equipos.cod_tipo_equipos = equipos.tipo_equipo
                WHERE saldo_pago >0 AND pagos.cod_contrato = '" . $data['cod_contrato'] . "' 
                ORDER BY fecha_ini_pago ASC";
        $result = mysqli_query($this->link, $query);
        $data   = array();
        while ($data[] = mysqli_fetch_assoc($result));
    ...
    public function consultaPagoAdelantado($data)
    {
    ...
    {
        $query  = "INSERT INTO factura (`fech_factura`,`fecha_registro`,`total_deuda`,`total_calculado`,`descuento`,`total_factura`,`tipo_factura`,`estado_factura`,`saldo_deuda`,`forma_pago`, `cliente`,`anticipado`) VALUES ('" . $data['fech_factura']  . "','" . date('Y-m-d')  . "', '". $data['total_deuda'] ."','". $data['total_deuda'] ."','0','". $data['total_factura'] ."',1,0,'". $data['saldo_deuda'] ."','". $data['forma_pago'] ."','". $data['cliente'] ."',1)";
        $result = mysqli_query($this->link, $query);
        if (mysqli_affected_rows($this->link) > 0) {
                return true;
        } else {
    ...
    //Edita pagos
    public function modificaPagoAdelantados($data)
    {
        $bandera = false;
        $diferencia = 0;
        for($i=0; $i< $data["contador"]; $i++){
            ...
            $query  = "UPDATE pagos SET valor_tot_pago = '".$data['valor_tot_pago_'.$i]."', valor_descuento = '". $data['valor_descuento_'.$i ] ."', valor_calculado = '". $valor_con_descuento ."', valor_recibido = '". $valor_recibido ."', saldo_pago = '". $saldo ."', estado_pago = '". $estado_pago ."', facturado=1 WHERE cod_equipo = '". $data['cod_equipo_'.$i ] ."' AND cod_pago = '". $data['cod_pago_'.$i ] ."'";
            $result = mysqli_query($this->link, $query);
            if (mysqli_affected_rows($this->link) > 0) {
                $bandera = true;
            } else {
    ...
    public function consultaFactura($data)
    {
        $query = "SELECT fech_factura, desc_tipo_pago, saldo_deuda AS total_saldo, total_calculado, total_factura AS total_recibido, total_deuda, descuento AS total_descuento, saldo, valor_recibido, valor_con_descuento, valor_descuento, valor_pago, fecha_fin_pago, fecha_ini_pago, desc_tipo_equipos, nom_equipo, nom_clase, consecutivo, cod_fac, CONCAT(nom1_cli,' ', nom2_cli, ' ', apel1_cli, ' ', apel2_cli) AS nombre_cliente, cedula_cli, direccion_cli, telefono_cli, celular_cli, email_cli FROM factura 
                ...
                WHERE cod_fac = '".$data['cod_fac']."' ORDER BY fecha_ini_pago ASC";
        $result = mysqli_query($this->link, $query);
        $data   = array();
        while ($data[] = mysqli_fetch_assoc($result));
        array_pop($data);
