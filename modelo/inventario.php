...
class Inventario
{
    ...
    //Trae los inventarioes
    public function getInventario()
    {
        $query  = "call consultar_inventarios()";
    ...
    public function insertaPaciente($data, $dataInventario)
    {
        $query  = "INSERT INTO `paciente` (nom1_pac, nom2_pac, apel1_pac, apel2_pac, cedula_pac, tipo_empleo_pac, direccion_pac, telefono_pac, ciudad_pac, celular_pac, fecha_ingreso, cod_inventario, cod_parentesco) VALUES ('" . $data['nom1_cli']  . "', '" . $data['nom2_cli']  . "', '" . $data['apel1_cli']  . "', '" . $data['apel2_cli']  . "', '" . $data['cedula_cli']  . "', '" . $data['tipo_empleo_cli']  . "' , '" . $data['direccion_cli']  . "' , '" . $data['telefono_cli']  . "' , '" . $data['ciudad_cli']  . "', '" . $data['celular_cli']  . "', '". date('Y-m-d'). "', '" . $dataInventario[0]['cod_cli']  . "', 0)";
        $result = mysqli_query($this->link, $query);
        if (mysqli_affected_rows($this->link) > 0) {
            return true;
        } else {
    ...
    //Consulta ultimo inventario
    public function getUltimoInventario(){
        $query  = "SELECT cod_cli FROM inventario ORDER BY cod_cli DESC LIMIT 1";
        $result = mysqli_query($this->link, $query);
        $data   = array();
        while ($data[] = mysqli_fetch_assoc($result));
        array_pop($data);
